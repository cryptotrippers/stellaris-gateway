...
### Fixed
- Governance proposals could never actually reach execution: nothing wrote `governance_proposals.status` from `active` to `passed`/`rejected` after a voting window closed, so `ExecuteCard` (gated on `status === "passed"`) never rendered for any proposal — no `fund_asset`, `set_fee`, or `accrue` proposal could ever create an asset, change a fee, or record an accrual. Added `finalizeProposal` (`governance-vault.functions.ts`), operator/admin-gated, which recomputes the tally and eligible weight server-side and applies the exact same `computeOutcome` quorum/threshold rules the UI already shows voters; wired a "Finalise" action into `/governance/$sip`.
- `governance_proposals` had no RLS `INSERT` policy for a plain authenticated user (only admin-`ALL` and operator-`UPDATE` existed), so `createVaultProposal` — the real, validated proposal-creation path — was rejected by Postgres for any non-admin submitter, contradicting the documented "submitter or admin can propose" design. Added an own-author insert policy mirroring the existing `funding_requests`/`proposal_votes` ownership pattern.
- Removed `governance-submit.functions.ts` (`createProposal`): an unauthenticated server function that inserted into `governance_proposals` via the service-role client, bypassing RLS entirely, reachable from the primary "New Proposal" button on `/governance`. Its UI (`/governance/new`) also fabricated a "Certik audit" verification claim and a "₳1,000 deposit / 24h timelock" that no code enforced. `/governance/new` now submits a real `signal` proposal through `createVaultProposal`.

### Added
- Funding-request onboarding (`/funding/new`) now requires accepting versioned platform terms before submission (`src/lib/onboarding-terms.ts`, recorded as `terms_version`/`terms_accepted_at` on `funding_requests`, enforced both server-side and by a DB check constraint), shows a live "projected net APY" matrix computed from the exact on-chain fee formula (`projectApyMatrix` in `vault-fees.ts`) at a few illustrative gross-yield scenarios, and the request detail page (`/funding/$id`) surfaces the same matrix plus a clearer "Smart contract" section linking to the operator bootstrap step once governance has created the asset.
