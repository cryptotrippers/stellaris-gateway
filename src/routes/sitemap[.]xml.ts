  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        // Query the real assets table server-side. No fabricated fallback.
        let assetIds: string[] = [];
        try {
          ...
          const { data } = await supabase.from("assets").select("id").eq("funding_status", "open");
          assetIds = (data ?? []).map((r) => r.id as string);
        } catch {
          assetIds = [];
        }
        ...
        const entries = [
          ...
          { path: "/assets", priority: "0.7", changefreq: "weekly" as const },
          ...
          { path: "/invite", priority: "0.7", changefreq: "weekly" as const },
          ...assetIds.map((id) => ({ path: `/marketplace/${id}`, priority: "0.7", changefreq: "daily" as const })),
        ];
