...
import {
  ...
  MIN_INITIAL_DEPOSIT,
  MIN_POSITION_VALUE,
  withdrawFromSusdrVault,
  type VaultView,
} from "@/lib/susdr-position";
import { getSusdrVault } from "@/lib/susdr-vaults.functions";

function usdrUnits(n: bigint | string, dp = 2): string {
  const value = typeof n === "bigint" ? n : BigInt(n || "0");
  const whole = value / 1_000_000n;
  const frac = (value % 1_000_000n).toString().padStart(6, "0").slice(0, dp);
  return `${whole}.${frac}`;
}

/**
 * Depositor-side smart contract actions for the sUSDr vault: mint shares with
 * `Deposit`, redeem them with `Withdraw`. Every figure shown is read from the
 * vault's on-chain State and Position UTxOs — mirrors `YieldVaultActionsCard.tsx`.
 */
export function SusdrVaultActionsCard({ usdrPolicyId }: { usdrPolicyId: string }) {
  ...
  const registryQ = useQuery({
    queryKey: ["susdr-vault-registry", usdrPolicyId],
  ...
  const viewQ = useQuery<VaultView>({
    queryKey: ["susdr-vault-view", usdrPolicyId, wallet.address, registryAddress],
    queryFn: () => loadMyVaultView(usdrPolicyId, registryAddress),
    enabled: walletReady,
  ...
  const minUsdr = Number(isBootstrapDeposit ? MIN_INITIAL_DEPOSIT : MIN_POSITION_VALUE) / 1e6;
  ...
  async function run(kind: "deposit" | "withdraw") {
    ...
    try {
      if (kind === "deposit") {
        const units = BigInt(Math.round((Number(amount) || 0) * 1e6));
        const r = await depositToSusdrVault({ usdrPolicyId, amountUsdr: units, registryAddress });
        setTxHash(r.txHash);
      } else {
        const r = await withdrawFromSusdrVault({ usdrPolicyId, registryAddress });
        setTxHash(r.txHash);
      }
  ...
  return (
    <div className="card-institutional p-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[10px] uppercase tracking-widest text-primary">sUSDr vault · on-chain</div>
      ...
      {!walletReady ? (
        ...
        <>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <Stat label="Share price" value={`${view.sharePrice.toFixed(6)} USDr`} />
            <Stat label="Your shares" value={totalShares.toString()} />
            <Stat label="Redeemable" value={`${usdrUnits(totalRedeemable)} USDr`} />
          </div>

          <div className="mt-3 rounded-md border border-border bg-secondary/20 p-3 text-[11px] text-muted-foreground">
            sUSDr: <span className="text-foreground">proof of claim — not transferable value</span>.
            One sUSDr unit is minted per share to evidence your deposit, but redemption is
            authorised by your position&apos;s owner key, not by holding the token. Sending sUSDr to
            another wallet transfers no redemption right, and this vault offers no transfer or
            trading action for it.
          </div>

          <div className="mt-3 rounded-md border border-border bg-secondary/20 p-3 text-[11px] text-muted-foreground">
            Management fee <span className="text-foreground">{formatFeeBps(view.state.feeBps)}</span>{" "}
            on accounted USDr, settled at each accrual by minting shares to the treasury. Unclaimed
            treasury shares: <span className="tabular-nums text-foreground">{view.state.treasuryShares}</span>.
            Your redeemable value already reflects the fee settled so far; fee accrued since the last
          ...
          <div className="mt-5 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={busy !== null || view.state.paused || (Number(amount) || 0) < minUsdr}
            ...
            <p className="text-[11px] text-muted-foreground">
              Minimum {minUsdr} USDr{isBootstrapDeposit ? " for the first deposit into this vault" : ""}.
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-border pt-4">
            ...
            >
              {busy === "withdraw" ? <Loader2 className="h-4 w-4 animate-spin" /> : "Redeem full position"}
            </button>
            <button
