/**
 * Pure decoding + accounting helpers for the sUSDr vault.
 *
 * Kept separate from any `*.functions.ts` module for the same reason
 * `yield-chain-decode.ts` is: server-function modules must contain nothing
 * but imports, types, and the server-function declarations themselves — the
 * build strips handler bodies and would delete any runtime sibling declared
 * alongside them.
 *
...
// ---------------------------------------------------------------------------
// SusdrDatum — mirrors contracts/susdr-vault/validators/susdr_vault.ak
//   Position { owner, shares }                                        -> constructor 0
//   State { total_shares, total_assets, epoch, operators, threshold,
//           paused, fee_bps, treasury, treasury_shares, last_fee_time } -> constructor 1
//
// Ten fields, not eleven: unlike yield_vault's YieldDatum, there is no
// `receipt_policy` field — the sUSDr minting policy id is structurally the
// vault's own script hash (see DESIGN.md), so there is nothing mutable to
// pin in the datum or drift-check here.
// ---------------------------------------------------------------------------

export interface SusdrStateDatum {
  totalShares: string;
  totalAssets: string;
  epoch: number;
  operators: string[];
  threshold: number;
  paused: boolean;
  /** Annual management fee in basis points, charged on accounted USDr. */
  feeBps: number;
...
export interface SusdrPositionDatum {
  owner: string;
  shares: string;
}

/** Quantity of a specific native asset (policy id + hex asset name) in a UTxO's amount list. */
export function assetQuantityOf(
  utxo: { amount: BfAmount[] },
  policyId: string,
  assetNameHex: string,
): bigint {
  const unit = `${policyId}${assetNameHex}`;
  const entry = utxo.amount.find((a) => a.unit === unit);
...
export function decodeState(d: PlutusData): SusdrStateDatum | null {
...
export function decodePosition(d: PlutusData): SusdrPositionDatum | null {
...
/**
 * The single writer for the sUSDr vault's State datum (constructor index 1).
 * Every builder (bootstrap, deposit, withdraw, accrue, pause, rotate, set
 * fee, claim fee) must go through this function so its output always matches
 * `decodeState` field-for-field — mirrors `yield-chain-decode.ts`'s
 * `encodeStateDatum`, written specifically to close AUDIT.md O-02 (a datum
 * constructor hand-rolled in more than one place, so a field can silently
 * land in the wrong position with no build-time error).
 */
export function encodeStateDatum(lucidMod: unknown, state: SusdrStateDatum): string {
...
/** The single writer for a Position datum (constructor index 0). */
export function encodePositionDatum(lucidMod: unknown, owner: string, shares: bigint): string {
  const { Data, Constr } = lucidMod as LucidDataMod;
  return Data.to(new Constr(0, [owner, shares]));
}

/** Safely read a State datum from an inline datum hex string. */
export function readStateDatum(datumHex: string | null): SusdrStateDatum | null {
...
export function readPositionDatum(datumHex: string | null): SusdrPositionDatum | null {
  if (!datumHex) return null;
  try {
...
/**
 * Never silently trust the first State-shaped UTxO found at a vault address
 * — mirrors AUDIT.md O-03. With the validator's own "exactly one State input
 * and output" rule, a planted second State is unspendable on-chain, but the
 * decoder must not read one for display or for transaction building: a
 * second State means the address is ambiguous and an operator has to look.
 */
 ...
 * usual cause being a vault that was bootstrapped twice. Mirrors
 * `yield-chain-decode.ts`'s `selectCanonicalState` exactly: the most advanced
 * ledger wins (epoch, then accounted assets, then shares, then fee clock);
 * ties go to the State descended from the registered bootstrap tx; remaining
 * ties break lexicographically on `txHash#index`.
 */
export function selectCanonicalState<T>(
  candidates: T[],
  ref: (c: T) => { txHash: string; outputIndex: number; state: SusdrStateDatum },
...
export interface SusdrVaultChainState {
  address: string;
  found: boolean;
  state: SusdrStateDatum | null;
  sharePrice: number | null;
  stateUtxo: { txHash: string; outputIndex: number; lovelace: string; usdr: string } | null;
  positions: Array<SusdrPositionDatum & { txHash: string; outputIndex: number; lovelace: string; usdr: string }>;
...
/** Derive accrual events from an ordered series of observed vault states. Mirrors `deriveAccruals`. */
export function deriveAccruals(
  points: Array<{ tx: { tx_hash: string; block_height: number; block_time: number }; state: SusdrStateDatum }>,
): ChainAccrual[] {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const treasuryDelta = BigInt(curr.state.treasuryShares) - BigInt(prev.state.treasuryShares);
    if (
    ...
    accruals.push({
      ...
      amountUsdr: assetsDelta.toString(),
...
/**
 * Annualised return between the first and last accrual. Returns null with
 * fewer than two observations — an APY cannot be honestly quoted from a
 * single data point, so the UI must show "not enough history" instead.
 */
